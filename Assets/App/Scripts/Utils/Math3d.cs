...
public class Math3d : MonoBehaviour
{
    ...
    public static System.Tuple<Vector3, Quaternion> RotateAround(Vector3 position, Quaternion rotation, Vector3 center, Vector3 axis, float angle)
    {
        var pos = position;
        ...
        return new System.Tuple<Vector3, Quaternion>(position, rotation);
    }

    // Blue Axis. Don't forget to multiple by the Rotation!
    public static Quaternion RotateToFaceOnY(Transform touse, Quaternion rotation, Vector3 WorldPos)
    {
        Quaternion result;
    ...
    public static Quaternion RotateToFaceOnX(Transform touse, Quaternion rotation, Vector3 WorldPos)
    {
        Quaternion result;
        Vector3 relative = touse.InverseTransformPoint(WorldPos);
        //  X
    ...
    public static Quaternion RotateToFaceOnZ(Transform touse, Quaternion rotation, Vector3 WorldPos)
    {
        Quaternion result;
        Vector3 relative = touse.InverseTransformPoint(WorldPos);
        // Z
    ...
    // swing is goin from up/down - not the same as twist!
    // twist is how mush of a degress is around the given axis!
    // works only in range 0-180
    public static void DecomposeSwingTwist(Quaternion inRotation, Vector3 twistAxis, out Quaternion swingPivot, out Quaternion angleAroundAxis)
    {
        Vector3 rotationVector = new Vector3(inRotation.x, inRotation.y, inRotation.z); // (rotaiton axis) * cos(angle / 2)
        ...
        // rotatedAround always a small incarucy in one radian - 2,67324
    }
    private static Vector3 NormalizeNagatives(Vector3 inVector)
    {
        return new Vector3(
